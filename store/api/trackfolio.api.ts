...
import { IInstrumentShort, InstrumentResponse } from '@/lib/models/api.model'
...
export const trackfolioApi = createApi({
  ...
  endpoints: build => ({
    ...
    searchAssets: build.query<IInstrumentShort[], string>({
      ...
      transformResponse: (response: InstrumentResponse<IInstrumentShort>) => response.instruments
    }),
