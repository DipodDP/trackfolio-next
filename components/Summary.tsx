...
const Summary = () => {
  ...
  if (data?.data) {
    currencyPositions = data.data.positions.filter(
      ...
      {isLoading ? (
        ...
        <div ref={myRef} className="grid gap-4">
          <div className="space-y-2">
            <h3 className="head-text text-left font-medium leading-none">
              Portfolio summary
            </h3>
          ...
          <div className="grid gap-2 w-fit">
            <div className="grid grid-cols-2 items-center gap-4">
              ...
              <div id="totalAdditionalCash" className="col-span-1 h-6 w-20">
                {data?.data ? (
                  formatCurrency(data.data.total_additional_cash)
                ) : (
                  <Loader />
                )}
              </div>
              ...
              <div id="totalAdditionalCash" className="col-span-1 h-6 w-20">
                {data?.data ? (
                  formatCurrency(data.data.total_amount_portfolio)
                ) : (
                  <Loader />
                )}
              </div>
